...
class MyProjectsManager {
  ...
  async saveWidgetEdits() {
    try {
      ...
      await (await import("https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js")).updateDoc(
        (await import("https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js")).doc(db, "widgets", id),
        {
          title,
          description,
          tags,
          updatedAt: (await import("https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js")).serverTimestamp?.() || new Date(),
        }
      );
  ...
  async performMove() {
    try {
      ...
      const slot = parseInt(document.getElementById("moveSlotSelect").value, 10);
