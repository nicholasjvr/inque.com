import{c as g}from"./cloud-upload-BHtgTKg3.js";import{_ as y}from"./auth-DH_qk2Bl.js";import{C as S}from"./css-analyzer-dJ6kCf09.js";import"https://www.gstatic.com/firebasejs/11.10.0/firebase-functions.js";import"https://www.gstatic.com/firebasejs/11.10.0/firebase-app.js";import"https://www.gstatic.com/firebasejs/11.10.0/firebase-analytics.js";import"https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/11.10.0/firebase-auth.js";import"https://www.gstatic.com/firebasejs/11.10.0/firebase-storage.js";class b{constructor(){this.previewContainer=null,this.currentFiles=new Map,this.fullscreenModal=null,this.fullscreenBody=null,this.currentFullscreenUrl=null,this.log("Widget Preview Manager initialized")}log(e,t=null){console.log(`[WIDGET PREVIEW] ${e}`,t||"")}error(e,t=null){console.error(`[WIDGET PREVIEW ERROR] ${e}`,t||"")}async init(){try{return this.log("Initializing widget preview system"),this.setupPreviewContainer(),this.setupFullscreenModal(),this.setupEventListeners(),!0}catch(e){return this.error("Failed to initialize widget preview system",e),!1}}setupPreviewContainer(){this.previewContainer=document.getElementById("widgetPreviewContainer"),this.previewContainer||(this.previewContainer=document.createElement("div"),this.previewContainer.id="widgetPreviewContainer",this.previewContainer.className="widget-preview-container",this.previewContainer.innerHTML=`
      ...
      `,document.body.appendChild(e)),this.fullscreenModal=e,this.fullscreenBody=e.querySelector("#fullscreenBody");const t=e.querySelector("#fullscreenCloseBtn");t&&!t._handlerBound&&(t.addEventListener("click",()=>this.hideFullscreen()),t._handlerBound=!0);const i=e.querySelector("#fullscreenOpenTabBtn");i&&!i._handlerBound&&(i.addEventListener("click",()=>{this.currentFullscreenUrl&&window.open(this.currentFullscreenUrl,"_blank")}),i._handlerBound=!0),e._backdropHandlerBound||(e.addEventListener("click",s=>{s.target===e&&this.hideFullscreen()}),e._backdropHandlerBound=!0),this._escHandlerBound||(document.addEventListener("keydown",s=>{s.key==="Escape"&&this.hideFullscreen()}),this._escHandlerBound=!0)}setupEventListeners(){const e=document.getElementById("previewCloseBtn");e&&e.addEventListener("click",()=>{this.hidePreview()}),document.querySelectorAll(".preview-tab").forEach(d=>{d.addEventListener("click",v=>{this.switchTab(v.target.dataset.tab)})});const i=document.getElementById("previewResponsiveBtn");i&&i.addEventListener("click",()=>{this.toggleResponsiveMode()});const s=document.getElementById("previewFullscreenBtn");s&&s.addEventListener("click",()=>{this.openFullscreenFromElement(document.getElementById("previewFrame"),"Widget Preview")});const a=document.getElementById("previewSaveBtn");a&&a.addEventListener("click",()=>{this.saveChanges()});const n=document.getElementById("fileSelector");n&&n.addEventListener("change",d=>{this.loadFileForEditing(d.target.value)});const r=document.getElementById("editorFormatBtn");r&&r.addEventListener("click",()=>{this.formatCode()}),document.querySelectorAll(".tool-btn").forEach(d=>{d.addEventListener("click",v=>{this.executeToolAction(v.target.dataset.tool)})});const o=document.getElementById("runAnalysisBtn");o&&o.addEventListener("click",()=>{this.runWidgetAnalysis()}),document.querySelectorAll(".style-preset").forEach(d=>{d.addEventListener("click",v=>{this.applyStylePreset(v.target.closest(".style-preset").dataset.preset)})});const l=document.getElementById("primaryColor");l&&l.addEventListener("change",d=>{this.updateStyleVariable("--primary-color",d.target.value)});const p=document.getElementById("backgroundStyle");p&&p.addEventListener("change",d=>{this.updateBackgroundStyle(d.target.value)});const h=document.getElementById("borderRadius");h&&h.addEventListener("input",d=>{this.updateStyleVariable("--border-radius",d.target.value+"px")}),this.previewContainer.addEventListener("click",d=>{d.target===this.previewContainer&&this.hidePreview()});const m=document.getElementById("codeEditor");if(m){let d;m.addEventListener("input",()=>{clearTimeout(d),d=setTimeout(()=>{this.autoSave()},1e3)})}}showPreview(){this.previewContainer&&(this.previewContainer.style.display="block",this.log("Preview shown"))}hidePreview(){this.previewContainer&&(this.previewContainer.style.display="none",this.clearPreview(),this.log("Preview hidden"))}clearPreview(){const e=document.getElementById("previewContent");e&&(e.innerHTML=`
        <div class="preview-placeholder">
          <span class="preview-icon">👁️</span>
          <p>Select files to preview your widget</p>
        </div>
      `),this.currentFiles.clear()}async handleFiles(e){try{this.log("Handling files for preview",{count:e.length});const t=Array.from(e);this.currentFiles.clear(),t.forEach(s=>{this.currentFiles.set(s.name,s)}),this.populateFileTree(t);const i=t.filter(s=>s.name.match(/\.html?$/i));if(i.length>0)await this.showProjectPreview(t,i[0]);else for(const s of t)await this.showFilePreview(s);this.showPreview()}catch(t){this.error("Error handling files for preview",t),this.showError("Failed to preview files: "+t.message)}}populateFileTree(e){const t=document.getElementById("fileList"),i=document.getElementById("fileSelector");if(t&&i){i.innerHTML='<option value="">Select a file to edit...</option>';const s=e.filter(l=>l.name.match(/\.html?$/i)),a=e.filter(l=>l.name.match(/\.css$/i)),n=e.filter(l=>l.name.match(/\.js$/i)),r=e.filter(l=>l.name.match(/\.(png|jpg|jpeg|gif|svg|webp)$/i)),c=e.filter(l=>!l.name.match(/\.(html?|css|js|png|jpg|jpeg|gif|svg|webp)$/i));let o="";s.length>0&&(o+=`<div class="file-category">
          <div class="category-header">📄 HTML (${s.length})</div>
          <div class="category-files">`,s.forEach(l=>{o+=`<div class="file-item" data-file="${l.name}">
            <span class="file-icon">📄</span>
            <span class="file-name">${l.name}</span>
            <span class="file-size">${this.formatFileSize(l.size)}</span>
          </div>`,i.innerHTML+=`<option value="${l.name}">📄 ${l.name}</option>`}),o+="</div></div>"),a.length>0&&(o+=`<div class="file-category">
          <div class="category-header">🎨 CSS (${a.length})</div>
          <div class="category-files">`,a.forEach(l=>{o+=`<div class="file-item" data-file="${l.name}">
            <span class="file-icon">🎨</span>
            <span class="file-name">${l.name}</span>
            <span class="file-size">${this.formatFileSize(l.size)}</span>
          </div>`,i.innerHTML+=`<option value="${l.name}">🎨 ${l.name}</option>`}),o+="</div></div>"),n.length>0&&(o+=`<div class="file-category">
          <div class="category-header">⚡ JavaScript (${n.length})</div>
          <div class="category-files">`,n.forEach(l=>{o+=`<div class="file-item" data-file="${l.name}">
            <span class="file-icon">⚡</span>
            <span class="file-name">${l.name}</span>
            <span class="file-size">${this.formatFileSize(l.size)}</span>
          </div>`,i.innerHTML+=`<option value="${l.name}">⚡ ${l.name}</option>`}),o+="</div></div>"),r.length>0&&(o+=`<div class="file-category">
          <div class="category-header">🖼️ Images (${r.length})</div>
          <div class="category-files">`,r.forEach(l=>{o+=`<div class="file-item" data-file="${l.name}">
            <span class="file-icon">🖼️</span>
            <span class="file-name">${l.name}</span>
            <span class="file-size">${this.formatFileSize(l.size)}</span>
          </div>`,i.innerHTML+=`<option value="${l.name}">🖼️ ${l.name}</option>`}),o+="</div></div>"),c.length>0&&(o+=`<div class="file-category">
          <div class="category-header">📁 Other (${c.length})</div>
          <div class="category-files">`,c.forEach(l=>{o+=`<div class="file-item" data-file="${l.name}">
            <span class="file-icon">📄</span>
            <span class="file-name">${l.name}</span>
            <span class="file-size">${this.formatFileSize(l.size)}</span>
          </div>`,i.innerHTML+=`<option value="${l.name}">📄 ${l.name}</option>`}),o+="</div></div>"),t.innerHTML=o,t.querySelectorAll(".file-item").forEach(l=>{l.addEventListener("click",()=>{const p=l.dataset.file;i.value=p,this.loadFileForEditing(p),this.switchTab("edit")})}),this.suggestAndDisplayTools(e)}}async showFilePreview(e){const t=document.getElementById("previewContent");if(!t)return;const i=URL.createObjectURL(e);let s;try{if(e.type.startsWith("image/"))s=document.createElement("img"),s.src=i,s.className="preview-image",s.alt=e.name;else if(e.type.startsWith("audio/"))s=document.createElement("audio"),s.controls=!0,s.src=i,s.className="preview-audio";else if(e.type.startsWith("video/"))s=document.createElement("video"),s.controls=!0,s.src=i,s.className="preview-video";else if(e.name.match(/\.html?$/i))s=document.createElement("iframe"),s.src=i,s.className="preview-iframe",s.sandbox="allow-scripts allow-same-origin";else if(e.name.match(/\.(js|css|json|txt)$/i)){const o=await this.readFileAsText(e);s=this.createCodePreview(o,e.name)}else s=document.createElement("div"),s.className="preview-unsupported",s.innerHTML=`
          <span class="file-icon">📄</span>
          <p>${e.name}</p>
      ...
      `;const n=document.createElement("div");n.className="preview-item",n.appendChild(a),n.appendChild(s);const r=document.createElement("div");r.className="preview-controls";const c=document.createElement("button");c.className="preview-fullscreen-btn",c.textContent="⛶ Fullscreen",c.addEventListener("click",()=>{const o=`Preview: ${e.name}`;this.openFullscreenFromElement(s,o,s.src||i)}),r.appendChild(c),n.appendChild(r),t.appendChild(n)}catch(a){this.error("Error creating preview for file",{fileName:e.name,error:a})}}async showProjectPreview(e,t){const i=document.getElementById("previewContent");if(i)try{const s={};e.forEach(m=>s[m.name]=m);let n=await this.readFileAsText(t);n=n.replace(/<link[^>]+href="([^"]+\.css)"[^>]*>/gi,async(m,d)=>s[d]?`<style>${await this.readFileAsText(s[d])}</style>`:m),n=n.replace(/<script[^>]+src="([^"]+\.js)"[^>]*><\/script>/gi,async(m,d)=>s[d]?`<script>${await this.readFileAsText(s[d])}<\/script>`:m);const r=new Blob([n],{type:"text/html"}),c=URL.createObjectURL(r),o=document.createElement("iframe");o.src=c,o.className="preview-iframe project-preview",o.sandbox="allow-scripts allow-same-origin allow-forms",o.title="Widget Preview";const u=document.createElement("div");u.className="project-info",u.innerHTML=`
        <h4>Project Preview</h4>
        <p>${e.length} files loaded</p>
        <ul class="file-list">
          ${e.map(m=>`<li>${m.name} (${this.formatFileSize(m.size)})</li>`).join("")}
        </ul>
      `;const l=document.createElement("div");l.className="preview-item project-preview-item",l.appendChild(u),l.appendChild(o);const p=document.createElement("div");p.className="preview-controls";const h=document.createElement("button");h.className="preview-fullscreen-btn",h.textContent="⛶ Fullscreen",h.addEventListener("click",()=>{this.openFullscreenFromElement(o,"Project Preview",c)}),p.appendChild(h),l.appendChild(p),i.appendChild(l)}catch(s){this.error("Error creating project preview",s),this.showError("Failed to preview project: "+s.message)}}openFullscreenFromElement(e,t="Preview",i=null){try{this.log("Opening fullscreen",{title:t,url:i}),this.fullscreenModal||this.setupFullscreenModal();const s=this.fullscreenModal.querySelector("#fullscreenTitle");s&&(s.textContent=t),this.fullscreenBody.innerHTML="";let a=null;const n=(e.tagName||"").toLowerCase();n==="iframe"?(a=document.createElement("iframe"),a.src=i||e.src,a.className="fullscreen-iframe",a.sandbox=e.sandbox||"allow-scripts allow-same-origin allow-forms"):n==="img"?(a=document.createElement("img"),a.src=i||e.src,a.className="fullscreen-image",a.alt=t):n==="video"?(a=document.createElement("video"),a.src=i||e.src,a.controls=!0,a.autoplay=!0,a.className="fullscreen-video"):n==="audio"?(a=document.createElement("audio"),a.src=i||e.src,a.controls=!0,a.autoplay=!0,a.className="fullscreen-audio"):(a=e.cloneNode(!0),a.classList.add("fullscreen-generic")),this.currentFullscreenUrl=i||e.src||null,this.fullscreenBody.appendChild(a),this.fullscreenModal.style.display="flex",document.body.style.overflow="hidden"}catch(s){this.error("Failed to open fullscreen",s)}}hideFullscreen(){this.fullscreenModal&&(this.fullscreenModal.style.display="none",this.fullscreenBody&&(this.fullscreenBody.innerHTML=""),document.body.style.overflow="",this.currentFullscreenUrl=null,this.log("Fullscreen closed"))}createCodePreview(e,t){const i=document.createElement("div");i.className="code-preview";const s=document.createElement("div");s.className="code-header",s.innerHTML=`
...
`)}executeToolAction(e){switch(e){case"format":this.formatCode();break;case"optimize":this.optimizeCSS();break;case"validate":this.validateHTML();break;case"preview":this.switchTab("preview");break;default:this.showToast(`Tool ${e} not implemented yet`,"info")}}optimizeCSS(){this.showToast("CSS optimization completed!","success")}validateHTML(){this.showToast("HTML validation completed!","success")}async runWidgetAnalysis(){const e=document.getElementById("analysisResults");if(e)try{e.innerHTML=`
    ...
    `,(document.querySelector(".toast-container")||document.body).appendChild(i),setTimeout(()=>{i.parentNode&&i.remove()},3e3)}async suggestTools(e){try{const{default:t}=await y(async()=>{const{default:c}=await import("./tools-filter-qyJYBFYl.js");return{default:c}},[],import.meta.url),i=[],s=new Set;return e.forEach(c=>{c.name.match(/\.html?$/i)&&s.add("html"),c.name.match(/\.css$/i)&&s.add("css"),c.name.match(/\.js$/i)&&s.add("javascript"),c.name.match(/\.react?\.js$/i)&&s.add("react"),c.name.match(/\.vue?\.js$/i)&&s.add("vue")}),[...t.getToolsByCategory("ai-ides"),...t.getToolsByCategory("design"),...t.getToolsByCategory("productivity"),...t.getToolsByCategory("templates")].filter(c=>{const o=c.features||[],u=c.tags||[];return s.has("html")&&(o.includes("HTML")||u.includes("HTML"))||s.has("css")&&(o.includes("CSS")||u.includes("CSS"))||s.has("javascript")&&(o.includes("JavaScript")||u.includes("JavaScript"))||s.has("react")&&(o.includes("React")||u.includes("React"))||s.has("vue")&&(o.includes("Vue")||u.includes("Vue"))||o.includes("Development")||u.includes("AI")||u.includes("Code Editor")}).sort((c,o)=>(o.rating||0)-(c.rating||0)).slice(0,3)}catch(t){return this.error("Error getting tool suggestions",t),[]}}displayToolSuggestions(e){const t=document.getElementById("toolButtons");if(!t)return;t.querySelectorAll(".suggested-tool").forEach(s=>s.remove()),e.forEach((s,a)=>{const n=document.createElement("button");n.className="tool-btn suggested-tool",n.title=`${s.name} - ${s.description}`,n.innerHTML=`
        <span>${s.icon}</span>
        <span>${s.name}</span>
      `,n.dataset.tool="suggested",n.dataset.toolName=s.name,n.dataset.toolUrl=s.url,n.addEventListener("click",r=>{this.openTool(s.name,s.url)}),t.appendChild(n)})}openTool(e,t){window.open(t,"_blank"),this.showToast(`Opened ${e} in new tab!`,"info"),this.log(`Opened tool: ${e} at ${t}`)}async suggestAndDisplayTools(e){try{const t=await this.suggestTools(e);this.displayToolSuggestions(t),t.length>0&&this.log(`Suggested ${t.length} tools for uploaded files`)}catch(t){this.error("Failed to suggest tools",t)}}}const f=new b;document.addEventListener("DOMContentLoaded",()=>{f.init(),window.widgetPreviewManager=f});class C{constructor(){this.uploadingSlots=new Set,this.cssAnalyzer=new S,this.log("Widget Upload Manager initialized with Cloud Functions and CSS Analyzer")}log(e,t=null){console.log(`[WIDGET UPLOAD] ${e}`,t||"")}error(e,t=null){console.error(`[WIDGET UPLOAD ERROR] ${e}`,t||"")}async init(){try{return this.log("Initializing widget upload system"),this.log("Widget upload system initialized"),this.setupEventListeners(),!0}catch(e){return this.error("Failed to initialize widget upload system",e),!1}}setupEventListeners(){this.log("Setting up event listeners"),document.querySelectorAll(".upload-widget-btn").forEach(a=>{a.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),this.handleUploadButtonClick(a)})}),document.querySelectorAll(".preview-widget-btn").forEach(a=>{a.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),this.handlePreviewButtonClick(a)})}),document.querySelectorAll(".widget-file-input").forEach(a=>{a.addEventListener("change",n=>{this.handleFileInputChange(n.target)})}),document.querySelectorAll(".widget-upload-area").forEach(a=>{this.setupDragAndDrop(a)}),this.log("Event listeners setup complete")}setupDragAndDrop(e){const t=e.querySelector(".widget-file-input");e.addEventListener("dragover",i=>{i.preventDefault(),e.style.borderColor="var(--primary-color)",e.style.background="rgba(0, 240, 255, 0.1)"}),e.addEventListener("dragleave",i=>{i.preventDefault(),e.style.borderColor="#555",e.style.background="#0f0f0f"}),e.addEventListener("drop",i=>{i.preventDefault(),e.style.borderColor="#555",e.style.background="#0f0f0f";const s=i.dataTransfer.files;s.length>0&&(t.files=s,this.handleFileInputChange(t))}),e.addEventListener("click",i=>{i.target.closest(".file-list")||i.target.closest(".action-buttons")||(t.value="",t.click())})}async handleFileInputChange(e){const t=e.files;t.length>0&&(this.log("Files selected",{count:t.length}),this.updateFileList(e,t),await this.analyzeWidgetFiles(t,e),f.handleFiles(t))}updateFileList(e,t){const i=e.closest(".widget-upload-area"),s=i.querySelector(".upload-placeholder p");if(t.length>0){s.textContent=`${t.length} file(s) selected`;let a=i.querySelector(".file-list");a||(a=document.createElement("div"),a.className="file-list",i.appendChild(a)),a.innerHTML="",Array.from(t).forEach(n=>{const r=document.createElement("div");r.className="file-item",r.innerHTML=`
        ...
        `,a.appendChild(r)}),this.addCSSAnalysisIndicator(i,t)}}getFileTypeClass(e){return e.match(/\.css$/i)?"css-file":e.match(/\.html?$/i)?"html-file":e.match(/\.js$/i)?"js-file":e.match(/\.(png|jpg|jpeg|gif|svg)$/i)?"image-file":"other-file"}getFileTypeLabel(e){return e.match(/\.css$/i)?"CSS":e.match(/\.html?$/i)?"HTML":e.match(/\.js$/i)?"JS":e.match(/\.(png|jpg|jpeg|gif|svg)$/i)?"IMG":"FILE"}addCSSAnalysisIndicator(e,t){const i=Array.from(t).filter(a=>a.name.match(/\.css$/i)),s=Array.from(t).filter(a=>a.name.match(/\.html?$/i));if(i.length>0||s.length>0){let a=e.querySelector(".css-analysis-indicator");a||(a=document.createElement("div"),a.className="css-analysis-indicator",a.innerHTML=`
