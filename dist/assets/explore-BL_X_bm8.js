import"./auth-BZnK8mjA.js";import{initializeApp as x}from"https://www.gstatic.com/firebasejs/11.10.0/firebase-app.js";import{getFirestore as U,getDocs as R,collection as D,getDoc as T,doc as m,updateDoc as h,arrayRemove as v,arrayUnion as L}from"https://www.gstatic.com/firebasejs/11.10.0/firebase-firestore.js";import{getAuth as M,onAuthStateChanged as P}from"https://www.gstatic.com/firebasejs/11.10.0/firebase-auth.js";import"./profile_banner-D4bpQstc.js";import"https://www.gstatic.com/firebasejs/11.10.0/firebase-analytics.js";import"https://www.gstatic.com/firebasejs/11.10.0/firebase-storage.js";const $={apiKey:"AIzaSyBIZcD-L5jD84hEYLxWOwHTE2iTY6EJ0zI",authDomain:"inque-31cb5.firebaseapp.com",projectId:"inque-31cb5",storageBucket:"inque-31cb5.firebasestorage.app",messagingSenderId:"338722493567",appId:"1:338722493567:web:4c46ecdfe92ddf2a5d5b4a",measurementId:"G-KQT58LWVSK"},O=x($),g=U(O),B=M(),a=document.getElementById("exploreWidgetList");a.innerHTML='<div style="color:#fff">Loading widgets...</div>';let i=null,d=new Set;async function p(){try{console.log("[EXPLORE DEBUG] Fetching widgets from widgets collection");const t=await R(D(g,"widgets")),e=[];return t.forEach(o=>{const s=o.data();console.log("[EXPLORE DEBUG] Processing widget",{id:o.id,title:s.title,userId:s.userId,fileCount:s.files?.length||0}),e.push({...s,id:o.id,userId:s.userId})}),console.log("[EXPLORE DEBUG] Total widgets found:",e.length),e}catch(t){return console.error("[EXPLORE DEBUG] Error fetching widgets:",t),[]}}function E(t){for(let e=t.length-1;e>0;e--){const o=Math.floor(Math.random()*(e+1));[t[e],t[o]]=[t[o],t[e]]}return t}function f(t){if(!Array.isArray(t)||t.length===0)return null;const e=t.find(o=>o.fileName&&/index\.html?$/i.test(o.fileName));return e||t.find(o=>o.fileName&&/\.html?$/i.test(o.fileName))}async function S(){if(i)try{console.log("[EXPLORE DEBUG] Loading user following list",{userId:i.uid});const t=await T(m(g,"users",i.uid));if(t.exists()){const e=t.data();d=new Set(e.following||[]),console.log("[EXPLORE DEBUG] Following list loaded",{count:d.size})}}catch(t){console.error("[EXPLORE DEBUG] Error loading following list:",t)}}async function I(t,e){if(!i){r("Please log in to follow users","warning");return}if(i.uid===t){r("You can't follow yourself","warning");return}try{const o=d.has(t);console.log("[EXPLORE DEBUG] Toggling follow",{targetUserId:t,isFollowing:o,currentUserId:i.uid}),o?(await h(m(g,"users",i.uid),{following:v(t)}),await h(m(g,"users",t),{followers:v(i.uid)}),d.delete(t),e.textContent="Follow",e.classList.remove("following"),r("Unfollowed user","info")):(await h(m(g,"users",i.uid),{following:L(t)}),await h(m(g,"users",t),{followers:L(i.uid)}),d.add(t),e.textContent="Following",e.classList.add("following"),r("Now following user!","success"))}catch(o){console.error("[EXPLORE DEBUG] Error toggling follow:",o),r("Error updating follow status","error")}}function F(t,e){if(!i){r("Please log in to message users","warning");return}if(i.uid===t){r("You can't message yourself","warning");return}console.log("[EXPLORE DEBUG] Opening message modal",{targetUserId:t,targetUserName:e});const o=document.createElement("div");o.className="message-modal",o.innerHTML=`
  ...
  `,document.body.appendChild(o),o.style.display="flex";const s=o.querySelector(".message-modal-close"),n=o.querySelector(".message-cancel-btn"),l=o.querySelector(".message-send-btn"),u=o.querySelector("#messageText"),c=()=>{document.body.removeChild(o)};s.addEventListener("click",c),n.addEventListener("click",c),o.addEventListener("click",w=>{w.target===o&&c()}),l.addEventListener("click",async()=>{const w=u.value.trim();if(!w){r("Please enter a message","warning");return}try{console.log("[EXPLORE DEBUG] Sending message",{to:t,from:i.uid,message:w}),r(`Message sent to ${e}!`,"success"),c()}catch(y){console.error("[EXPLORE DEBUG] Error sending message:",y),r("Error sending message","error")}}),setTimeout(()=>u.focus(),100)}function r(t,e="info"){const o=document.createElement("div");o.className=`explore-toast explore-toast-${e}`,o.textContent=t,document.body.appendChild(o),setTimeout(()=>{o.parentNode&&o.parentNode.removeChild(o)},3e3)}(async function(){try{let e=await p();if(!e.length){a.innerHTML='<div style="color:#fff">No widgets found yet.</div>';return}const o=e.filter(s=>{const n=f(s.files);return n&&n.downloadURL});if(!o.length){a.innerHTML='<div style="color:#fff">No widgets with HTML files found.</div>';return}e=E(o).slice(0,12),a.innerHTML="";for(const s of e){const n=f(s.files);console.log("[EXPLORE DEBUG] Rendering widget",{id:s.id,title:s.title,htmlFile:n?.fileName,downloadURL:n?.downloadURL});const l=document.createElement("div");l.className="explore-widget-card";const u=d.has(s.userId),c=i&&i.uid===s.userId;l.innerHTML=`
      ...
      `,a.appendChild(l)}}catch(e){console.error("[EXPLORE DEBUG] Error loading widgets:",e),a.innerHTML=`<div style='color:#f66'>Error loading widgets: ${e.message}</div>`}})();P(B,async t=>{if(console.log("[EXPLORE DEBUG] Auth state changed",{userId:t?.uid}),i=t,t){await S();const e=await p();if(e.length>0){const o=e.filter(n=>{const l=f(n.files);return l&&l.downloadURL}),s=E(o).slice(0,12);b(s)}}else{d.clear();const e=await p();if(e.length>0){const o=e.filter(n=>{const l=f(n.files);return l&&l.downloadURL}),s=E(o).slice(0,12);b(s)}}});document.addEventListener("click",t=>{if(t.target.matches(".explore-follow-btn")){t.preventDefault();const e=t.target.dataset.userId;I(e,t.target)}if(t.target.matches(".explore-message-btn")){t.preventDefault();const e=t.target.dataset.userId,o=t.target.dataset.userName;F(e,o)}});async function b(t){a.innerHTML="";for(const e of t){const o=f(e.files);console.log("[EXPLORE DEBUG] Rendering widget",{id:e.id,title:e.title,htmlFile:o?.fileName,downloadURL:o?.downloadURL});const s=document.createElement("div");s.className="explore-widget-card";const n=d.has(e.userId),l=i&&i.uid===e.userId;s.innerHTML=`
