...
exports.uploadWidgetFiles = functions.https.onCall(async (data, context) => {
...
exports.uploadProfilePhoto = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    // Create storage reference
    const fileRef = storage.bucket().file(`users/${userId}/profile-photo.jpg`);
    ...
    console.log(`[UPLOAD PROFILE] Profile photo uploaded for user: ${userId}`);
...
exports.deleteWidget = functions.https.onCall(async (data, context) => {
...
exports.getWidgetDownloadUrls = functions.https.onCall(
  async (data, context) => {
...
// HTTP function for CORS testing
exports.testUpload = functions.https.onRequest((req, res) => {
  cors(req, res, () => {
    res.json({
      message: "Upload functions are working!",
